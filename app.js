...
const mongodb_URI = 'mongodb+srv://sampu13:PalpaNepal123@sampada.2ibed.mongodb.net/myFirstDatabase?retryWrites=true&w=majority'
...
app.get('/foodreviewForm',
  (req, res, next) => {
    res.render("foodreviewForm");
  });

  app.get('/foodreview',
  async (req, res, next) => {
    const items = await Foodreview.find({})
    res.json(items)
    //res.send('foodreview')
  }
)
...
app.post('/foodreview',
  isLoggedIn,
  async (req, res, next) => {
    const { foodreview } = req.body
    const item = new Foodreview({
      userId: req.session.user._id,
      review: foodreview,
      createAt: new Date()
    })
    await item.save()
    res.redirect('/foodreview')
  }
)
